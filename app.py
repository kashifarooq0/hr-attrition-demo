...
def describe_change_plain_english(col, old_val, new_val):
    """Turns a raw feature change into a plain, HR-readable sentence."""
    if col == "OverTime":
        old_b, new_b = old_val == 1, new_val == 1
        if old_b and not new_b:
            return "Reduce or eliminate overtime work"
        if not old_b and new_b:
            return "Employee starts working overtime"
        return None
    ...
    if col == "MonthlyIncome":
        ...
        if diff > 0:
            return f"Give a raise of about ${diff:,.0f}/month (~{pct:.0f}% increase)"
        return f"Lower monthly salary by about ${abs(diff):,.0f}/month"

    if col == "JobSatisfaction":
        if new_val > old_val:
            return f"Improve job satisfaction (from {old_val:.0f}/4 to {new_val:.0f}/4) — e.g. better role fit, recognition, or clearer growth path"
        return f"Job satisfaction would need to drop (from {old_val:.0f}/4 to {new_val:.0f}/4)"

    if col == "WorkLifeBalance":
        if new_val > old_val:
            return f"Improve work-life balance (from {old_val:.0f}/4 to {new_val:.0f}/4) — e.g. flexible hours or lighter workload"
        return f"Work-life balance would need to drop (from {old_val:.0f}/4 to {new_val:.0f}/4)"

    if col == "TrainingTimesLastYear":
        if new_val > old_val:
            return f"Offer more training (from {old_val:.0f} to {new_val:.0f} sessions per year)"
        return f"Offer less training (from {old_val:.0f} to {new_val:.0f} sessions per year)"

    if col == "DistanceFromHome":
        if new_val < old_val:
            return f"Reduce commute distance (from {old_val:.0f} to {new_val:.0f} miles) — e.g. remote/hybrid work or relocation support"
        return f"Commute distance would need to increase (from {old_val:.0f} to {new_val:.0f} miles)"

    if col == "JobInvolvement":
        if new_val > old_val:
            return f"Increase engagement in projects/responsibilities (from {old_val:.0f}/4 to {new_val:.0f}/4)"
        return f"Job involvement would need to drop (from {old_val:.0f}/4 to {new_val:.0f}/4)"

    return f"{col}: {old_val:g} → {new_val:g}"
...
if st.button("🔮 Predict risk & survival curve", type="primary"):
    ...
    with st.spinner("Searching for minimal, realistic changes..."):
        try:
            ...
            # IMPORTANT: always target class 0 ("stays") explicitly, rather than
            # desired_class="opposite". "opposite" flips relative to whatever the
            # model's current 0/1 call already is -- for an employee already
            # predicted to stay (risk < 50%), "opposite" would search for ways
            # to make risk WORSE. We always want the direction that lowers risk.
    ...
    if cf_df is not None and len(cf_df) > 0:
        ...
        for _, cf_row_result in cf_df.iterrows():
            plain_changes = []
            raw_changes = []
            for col in ACTIONABLE_FEATURES:
                ...
                if old_val != new_val:
                    desc = describe_change_plain_english(col, old_val, new_val)
                    if desc:
                        plain_changes.append(desc)
                    raw_changes.append((col, old_val, new_val))
            ...
            # Only keep scenarios that actually reduce risk. DiCE can return
            # candidates that satisfy the target class technically but don't
            # meaningfully help, or (with small/skewed data) even move the
            # wrong way -- filter those out rather than showing bad advice.
            if new_risk >= risk - 0.005:
                continue
            ...
            scenarios.append((plain_changes, new_risk, new_median_tenure))

        # Show the strongest (lowest resulting risk) scenarios first, deduped
        scenarios.sort(key=lambda s: s[1])
        seen = set()
        shown = 0

        for plain_changes, new_risk, new_median_tenure in scenarios:
            key = tuple(sorted(plain_changes))
            if key in seen or not plain_changes:
                continue
            seen.add(key)
            shown += 1
            ...
            if median_tenure is not None and new_median_tenure is not None:
                tenure_text = (
                    f"they'd be expected to stay about **{new_median_tenure - median_tenure:.1f} years longer** "
                    f"(from ~{median_tenure:.1f} to ~{new_median_tenure:.1f} years)"
                )
            elif new_median_tenure is not None:
                tenure_text = f"their expected tenure becomes about **{new_median_tenure:.1f} years**"
            else:
                tenure_text = "their expected tenure moves beyond what this model can estimate (a good sign)"

            risk_drop = risk - new_risk
            st.markdown(
                f"**Option {shown}:** " + " and ".join(plain_changes) + "\n\n"
                f"→ This would lower predicted risk from **{risk:.1%}** to **{new_risk:.1%}** "
                f"(a **{risk_drop:.1%}** drop), and {tenure_text}."
            )
        ...
        if shown == 0:
            st.info(
                "No realistic, bounded combination of the actionable levers "
                "(overtime, salary, job satisfaction, work-life balance, training, "
                "commute distance, job involvement) meaningfully lowers this employee's risk. "
                "Their risk is likely driven mainly by factors outside typical HR policy levers "
        ...
        st.info(
            "No realistic, bounded combination of the actionable levers "
            "(overtime, salary, job satisfaction, work-life balance, training, "
            "commute distance, job involvement) was found to lower this employee's risk. "
            "Their risk is likely driven mainly by factors outside typical HR policy levers "
            "(e.g. age, tenure, marital status)."
        )
